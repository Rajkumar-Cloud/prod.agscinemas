<?php
    ...
    if (isset($_REQUEST['MovieFailureRefund'])) {
        $movieId = $_POST['form'][2]['value'];
        $movieName = $_POST['form'][0]['value'];
        $show_date_time = strtr($_POST['form'][4]['value'], '/', '-');
        $movie_location = $_POST['form'][3]['value'];
        $movieSessionId = $_POST['form'][1]['value'];
        ...
        foreach ($transHistoryData as $key1 => $val) {  
            if(count($val) == 3 && $userLoc == 5){
                ...
                foreach ($getTransLog as $jsonkey => $json) {   
                    if((int)$json->movie_showid == (int)$movieSessionId && $json->Cin_Id == $movie_location){
                        $obj = array();
        ...
        /*if(count($array) > 0) {
            ...
            $jsonData = ['type'=>'success', 'message'=>"success", 'data'=>$array, 'code'=>'201'];
        } else {
            $jsonData = ['type'=>'fail', 'message'=>"records not found", 'data'=>$array, 'code'=>'401'];
        }*/
...
function getTransLog($trans_id)
{
    ...
    foreach ($Details as $key => $value) {
        ...
        $array->movie_showdatetime = $Details[0]->movie_date.' '.$Details[0]->movie_showtime; 
        $array->movie_showid = $Details[0]->movie_showid; 
