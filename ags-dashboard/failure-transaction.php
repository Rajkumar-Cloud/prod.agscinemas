<?php 
 $title = "Failure Transaction Reports";
 $description = "AGS Cinemas Failure Transaction List";
 $keywords = "AGS Cinemas Failure Transaction List";
 $menu_name = "Failure Transaction";
 $Sub_title = "Failure Sales Report";
 include('header.php'); 
?>
        ...
        <div class="card table-card mb-1">
			...
			<div class="card">
                        ...
                        <div class="table-responsive">
                            <table id="ags_failure_tbl" class="table display dataTable table-condensed mb-1">
                                <thead>
                                    <tr>
                                        <!-- <th>Booking ID</th> -->
                                        <!-- <th>VISTA ID</th> -->
...
$(document).ready(function() {
    var data_table = $('#ags_failure_tbl').DataTable({                   
            "ajax": "classes/failure_transaction.php",
            "ordering" : true,
            columnDefs: [ { type: 'date', 'targets': [16] }, { type: 'date', 'targets': [17], "visible": false, "searchable": true } ],
            "order": [[ 16, "asc" ]],
            ...
            $('#totalAmnt').html(`<b>Total:</b> ${$('#ags_failure_tbl').DataTable().column(13, {search:'applied'}).data().toArray().reduce((sum, item) => sum+=item, 0)}`);
            $('#totalConvFee').html(`<b>Total:</b> ${$('#ags_failure_tbl').DataTable().column(12, {search:'applied'}).data().toArray().reduce((sum, item) => sum+=item, 0)}`);
            $('#totalMovAmnt').html(`<b>Total:</b> ${$('#ags_failure_tbl').DataTable().column(11, {search:'applied'}).data().toArray().reduce((sum, item) => sum+=item, 0)}`);
            },
            initComplete: function () {
                this.api().columns().every( function () {
                    var column = this;
                    var ColIndex = column[0][0];
                    var title = $('#ags_failure_tbl thead th').eq( ColIndex ).text();
                    if(ColIndex != 9 && ColIndex != 10 && ColIndex != 11 && ColIndex != 12 && ColIndex != 13 && ColIndex != 14 && ColIndex != 15) {
        ...
        $('#ags_failure_tbl tfoot th').each( function (i) {
            var title = $('#ags_failure_tbl thead th').eq( $(this).index() ).text();
