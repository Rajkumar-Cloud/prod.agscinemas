...
(function($) {
	...
	$.fn.seatCharts = function (setup) {
		...
		var fn       = this,
			...
			seat = (function(seatCharts, seatChartsSettings) {
				return function (setup) {
                  ...
                  //  $('.seatCharts-seat').prepend('<img id="theImg" src="images/Available.png"/>');
		...
		settings.naming.rows = settings.naming.rows || (function(length) {
			var rows =rownames;  //Row names Array value pass in global value ["A","B"]//
		...
		if (settings.naming.top) {
			...
			if (settings.naming.left) {
              ...
              //  $('.seatCharts-cell').prepend('<img id="theImg" src="images/Available.png"/>');
			}
		...
		$.each(settings.map, function(row, characters) {
			 ...
			 */
			 ...
			 var numberorder=0;
			 var split_rowid = characters.split('//');
			 characters=split_rowid[0];
			 var seatrowid=split_rowid[1];
			 var actualcol = split_rowid[3].split(",");
			 var gridcol = split_rowid[2].split(",");			 
			$.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) { 
				var matches         = characterParams.match(/([a-z_]{1})(\[([0-9a-z_ ,]+)\])?/i),
					...
					if(character != 'Z'){//using Countable space (Z Using DB)			
								
				$row.append(character != '_' ?
					//if the character is not an underscore (empty space)
					(function(naming) {
	                var showvalue=naming.columns[column]-numberorder; 
	                var showvalue11=naming.columns[column]; 
...
// gridcol	
					...
					var id = overrideId ? overrideId : naming.getId(character, naming.rows[row],actualcol[column]+'//'+naming.rows[row]+''+gridcol[column]+'//'+seatrowid+'//'+column);
						seats[id] = new seat({
							id        : id,
							label     : overrideLabel ?
							overrideLabel : naming.getLabel(character, naming.rows[row], actualcol[column]),
							row       : row,
							column    : column,
							character : character
						});						
					...
					$('<div></div>').addClass('seatCharts-cell seatCharts-space')	
					    ...
					    var ttt = $('<div></div>').addClass('seatCharts-cell seatCharts-space');
					    $row.append(ttt);
					}
			});
